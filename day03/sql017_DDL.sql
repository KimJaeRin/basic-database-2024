...
/*
    ...
    CHAR(n)[고정문자], VARCHAR(n)[가변 문자열], NCHAR(n), NVARCHAR(n), N-[유니코드],
    예(username : CHAR(10)/VARCHAR(10) ) 'hugo'
    CHAR(10) = 'hugo      ' 빈 공간은 스페이스
    VARCHAR(10) = 'hugo'이름을 넣고 10개의 자리 중 빈 곳은 삭제
    DATE[날짜], DARETIME[날짜시간], DECIMAL(18, 0)[소수점 표현 실수], FLOAT[실수],
...
DROP TABLE NewCustomer; -- 불가능 (사용중이기때문)

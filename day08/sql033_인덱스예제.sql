-- 지난 주 잘못만든 데이터 초기화
-- 삭제
DELETE FROM Users; --WHERE절이 없으면 모두 삭제를 뜻함
...
-- 인덱스는 대량의 데이터에서 찾고자하는 데이터를 빨리 찾게해주는 목적
-- WHERE절, JOIN의 ON절에 들어가는 컬럼에 인덱스를 설정하는것의 속도개선에 도움
-- 1번 PK, 2번 FK, 3번 WHERE절에 검색시 들어가는 컬럼에 인덱스를 설정
-- 단, NULL값이 많거나 중복이 많으면 컬럼에 인덱스를 걸면 성능에 도움받지 못 함

-- 서브쿼리 리뷰
-- ALL, ANY(SOME), SOME(ANY) 쓰는 조건은 아래의 데이터에서는 사용이 어려움

-- 3번 고객이 주문한 도서의 최고금액보다 더 비싼 도서를 구입한 주문번호, 금액 표시
SELECT *
  FROM customer;
...
SELECT MAX(saleprice)
 ...
 -- 13,000원 보다 비싼 도서를 구입한 주문번호, 금액표시
 SELECT o1.orderid
      , o1.saleprice
   FROM Orders AS o1
...
-- SELECT절 서브쿼리, JOIN으로 변경 가능 이미 쿼리가 너무 복잠해서 더이상 테이블을 추가하기 힘들면 많이사용
-- 고객별 판매액을 보이시오, GROUP BY가 들어가면 SELECT정에 반드시 집계함수가 들어가야 함!!
...
-- 업데이트, 한꺼번에 필요한 필드값을 한 테이블에서 다른 테이블로 복사할때 아주 유용함.
UPDATE Orders
